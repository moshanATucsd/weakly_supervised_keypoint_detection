function heatmap = dagnn_occlusion(net, im_, box, inputName, outputName, varargin)
...
heatmap = occlude(net, inputName, im_, box, score, measureVariable, measureFilter, boxColor, sizesAndStrides{1:end});

net.mode = mode;
net.conserveMemory = conserveMemory;
end

function heatmap = occlude(net, inputName, im_, box, score, measureVariable, measureFilter, boxColor, varargin)
...
for k = 1:4:length(varargin)
    ...
    tempHeat = compute_heat(net, inputName, im_, box, score, measureVariable, measureFilter, boxColor, varargin{k}, varargin{k+1}, varargin{k+2}, varargin{k+3});
...
function heatmap = compute_heat(net, inputName, im_, box, score, measureVariable, measureFilter, boxColor, sizeW, strideW, sizeH, strideH)
...
startW = round(max(startW, box(1)));
endW = round(min(endW, box(1)+box(3)));
startH = round(max(startH, box(2)));
endH = round(min(endH, box(2)+box(4)));
