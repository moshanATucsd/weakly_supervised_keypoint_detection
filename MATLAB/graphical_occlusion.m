function [heatmap, mask, overlay] = graphical_occlusion(net, im, im_, box,varargin)
...
% the terms of the BSD license (see the LICENCE file).
    ...
    if isa(net, 'DDagNN') || isa(net, 'dagnn.DagNN')
        mask = [];
        [heatmap, overlay] = dagnn_graphical_occlusion(net, im, im_, box, varargin{:});
    else
        [heatmap, mask, overlay] = simplenn_graphical_occlusion(net, im, im_,box,varargin{:});
    end
end
