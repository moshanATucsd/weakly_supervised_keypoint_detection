function [heatmap_disp, mask] = simplenn_graphical_deconvolution(net, im, im_, box, varargin)
...
% the terms of the BSD license (see the LICENCE file).
    ...
    box_mask = zeros(h,w);
    box = round(box);
    box_mask(max(box(2),1):min(box(2)+box(4),h),max(box(1),1):min(box(1)+box(3),w)) = 1;
    for i = 1:3
        heatmap(:,:,i) = heatmap(:,:,i).*box_mask;
    end
    
    % --- display results ---
    [mask, heatmap_disp] = display(net, im, heatmap, class, classScore);
end

function [mask, im4] = display(net, im, heat, class, classScore)
