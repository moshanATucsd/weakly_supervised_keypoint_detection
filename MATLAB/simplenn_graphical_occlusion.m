function [heatmap, mask, overlay] = simplenn_graphical_occlusion(net, im, im_,box, varargin)
...
% the terms of the BSD license (see the LICENCE file).
    ...
    heatmap = simplenn_occlusion(net, im_, box, parameters{:});

    % --- display results ---
    [mask, overlay] = display(net, im, heatmap, class, classScore);
end

function [mask, overlay] = display(net, im, heat, class, classScore)
