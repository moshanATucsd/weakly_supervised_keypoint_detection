function [heatmap_disp, mask] = graphical_deconvolution(net, im, im_, box, varargin)
...
% the terms of the BSD license (see the LICENCE file).
    ...
    if isa(net, 'DDagNN') || isa(net, 'dagnn.DagNN')
        heatmap_disp = dagnn_graphical_deconvolution(net, im, im_, varargin{:});
    else
        [heatmap_disp, mask] = simplenn_graphical_deconvolution(net, im, im_, box, varargin{:});
    end
end
