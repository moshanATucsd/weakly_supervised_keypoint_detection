function [heatmap, overlay] = dagnn_graphical_occlusion(net, im, im_, box, varargin)
...
% the terms of the BSD license (see the LICENCE file).
    ...
    heatmap = dagnn_occlusion(net, im_, box, inputName, outputName, parameters{:});

    % --- display results ---
    overlay = display(net, im, heatmap, classIndex, classScore);
end

function overlay = display(net, im, heat, class, classScore)
